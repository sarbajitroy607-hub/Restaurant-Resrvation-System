...
const uploadToClodinary = (fileBuffer) => {
...
export const getOwnwerRestaurants = async (req, res) => {
    try {
...
export const createOwnerRestaurant = async (req, res) => {
    try {
        ...
        const parsedAvailableSlots = typeof availableSlots === "string" ? availableSlots.split(",").map((s) => s.trim()) : availableSlots;
...
export const updateOwnerRestaurant = async (req, res) => {
    try {
        ...
        if (tags !== undefined)
            restaurant.tags = parseCommaSeparatedValues(tags) ?? [];
        if (availableSlots !== undefined)
            restaurant.availableSlots = parseCommaSeparatedValues(availableSlots) ?? [];
